"""
BCMC-VRPHD Instance Loader
===========================
Reads an Excel (.xlsx) instance file into a structured Instance object.
"""
...
def load_instance(filepath: str) -> Instance:
    ...
    for _, row in df_veh.iterrows():
        for cls in row["capable_classes"].split(","):
            cls = cls.strip()
    ...
    for _, row in df_tt.iterrows():
        p[(int(row["from"]), int(row["to"]), int(row["vehicle_id"]))] = row["time_min"]
    ...
    for _, row in df_dem.iterrows():
        nid, cls, qty = int(row["node_id"]), row["class"], row["quantity"]
        q[(nid, cls)] = qty
        V_d.setdefault(cls, [])
    ...
    for _, row in df_sr.iterrows():
        r[(int(row["vehicle_id"]), row["class"])] = row["rate_units_per_min"]

    # ── Baseline ─────────────────────────────────────────────
    df_bl = pd.read_excel(xls, "Baseline")
    L_bar = dict(zip(df_bl["vehicle_id"].astype(int), df_bl["baseline_min"]))
